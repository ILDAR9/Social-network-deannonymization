...
class ExpandUserIdentity:
    ...
    def __init__(self, lg, rg, seeds_0, name_sim_threshold = 61, is_repeat=False, model = None):
        ...
        if model:
            ...
            self.model = model
            self.load_cache()
            self.decide_top = self.__decide_top_with_model
        else:
            self.decide_top = self.__decide_top_no_model
    ...
    def __get_top(self):
        ...
        while self.inactive_pairs:
            ...
            if self.decide_top(s):
                return s
            else:
                self.train_data[(self.lg.vs[s[0]]['uid'], self.rg.vs[s[1]]['uid'])] = s[2]
        return None

    def __decide_seed(self, l_neighbor, r_neighbor):
    ...
    def __decide_top_no_model(self, s):
        return not self.__in_matched(s[0], s[1])

    def __decide_top_with_model(self, s):
        lv = self.lg.vs[s[0]]
        rv = self.rg.vs[s[1]]
        ...
        # feature_set = []
        # feature_set.append(s[2])
        n_deg = lv.degree()
        m_deg = rv.degree()

        feature_set.append(abs(n_deg - m_deg) / max(n_deg, m_deg, 1))
        feature_set.append(abs(n_deg - m_deg))

        #
        degs_l = self.degs(lv)
        degs_r = self.degs(rv)

        # m_deg_l = np.mean(degs_l)
        # m_deg_r = np.mean(degs_r)
        # feature_set.append(abs(m_deg_l - m_deg_r) / max(m_deg_l, m_deg_r, 1))

        med_deg_l = np.median(degs_l)
        med_deg_r = np.median(degs_r)
        feature_set.append(abs(med_deg_l - med_deg_r))
